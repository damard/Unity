...
public class WheeledVehicle : MonoBehaviour {
	...
	void FixedUpdate () {
		...
		foreach (Axle axle in axles) {
			if (axle.isMotored) {
				axle.leftWheel.motorTorque = wheelTorque * forwardInput;
				axle.rightWheel.motorTorque = wheelTorque * forwardInput;
			}
			if (axle.isSteerable) {
				axle.leftWheel.steerAngle = axle.maxSteerAngle * steerInput;
				axle.rightWheel.steerAngle = axle.maxSteerAngle * steerInput;
			}
			if (isBraking) {
